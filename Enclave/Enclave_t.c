...
static sgx_status_t SGX_CDECL sgx_execute_secure_operation(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_execute_secure_operation_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_execute_secure_operation_t* ms = SGX_CAST(ms_execute_secure_operation_t*, pms);
	...
	execute_secure_operation(_in_sealed_data, _tmp_sealed_data_size, ms->ms_password);
	if (_in_sealed_data) {
...
static sgx_status_t SGX_CDECL sgx_initialize_enclave_data(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_initialize_enclave_data_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_initialize_enclave_data_t* ms = SGX_CAST(ms_initialize_enclave_data_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	uint8_t* _tmp_sealed_data = ms->ms_sealed_data;
	uint32_t _tmp_sealed_data_size = ms->ms_sealed_data_size;
	size_t _len_sealed_data = _tmp_sealed_data_size;
	...
	initialize_enclave_data(_in_sealed_data, _tmp_sealed_data_size, ms->ms_initial_value);
	if (_in_sealed_data) {
		if (memcpy_s(_tmp_sealed_data, _len_sealed_data, _in_sealed_data, _len_sealed_data)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_execute_secure_operation, 0, 0},
		{(void*)(uintptr_t)sgx_initialize_enclave_data, 0, 0},
...
sgx_status_t SGX_CDECL ocall__exit(int state)
{
	...
	ocalloc_size -= sizeof(ms_ocall__exit_t);
...
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_state = sizeof(int);

	ms_ocall_waitpid_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_waitpid_t);
	void *__tmp = NULL;

	void *__tmp_state = NULL;

	CHECK_ENCLAVE_POINTER(state, _len_state);

	if (ADD_ASSIGN_OVERFLOW(ocalloc_size, (state != NULL) ? _len_state : 0))
		return SGX_ERROR_INVALID_PARAMETER;
	...
	ocalloc_size -= sizeof(ms_ocall_waitpid_t);

	ms->ms_pid = pid;
	if (state != NULL) {
		ms->ms_state = (int*)__tmp;
		__tmp_state = __tmp;
		if (_len_state % sizeof(*state) != 0) {
			sgx_ocfree();
			return SGX_ERROR_INVALID_PARAMETER;
		}
		memset(__tmp_state, 0, _len_state);
		__tmp = (void *)((size_t)__tmp + _len_state);
		ocalloc_size -= _len_state;
	} else {
		ms->ms_state = NULL;
	}
	...
	if (status == SGX_SUCCESS) {
		if (retval) *retval = ms->ms_retval;
		if (state) {
			if (memcpy_s((void*)state, _len_state, __tmp_state, _len_state)) {
				sgx_ocfree();
				return SGX_ERROR_UNEXPECTED;
			}
		}
...
sgx_status_t SGX_CDECL ocall_fork(int* retval)
{
	...
	ocalloc_size -= sizeof(ms_ocall_fork_t);
...
sgx_status_t SGX_CDECL ocall_shmget(int* retval, key_t key, int size, int shmflg)
{
	...
	ms_ocall_shmget_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_shmget_t);
	void *__tmp = NULL;
	...
	ms = (ms_ocall_shmget_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_shmget_t));
	ocalloc_size -= sizeof(ms_ocall_shmget_t);
...
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_shmaddr = 200;

	ms_ocall_shmat_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_shmat_t);
	void *__tmp = NULL;


	CHECK_ENCLAVE_POINTER(shmaddr, _len_shmaddr);

	if (ADD_ASSIGN_OVERFLOW(ocalloc_size, (shmaddr != NULL) ? _len_shmaddr : 0))
		return SGX_ERROR_INVALID_PARAMETER;
	...
	ms = (ms_ocall_shmat_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_shmat_t));
	ocalloc_size -= sizeof(ms_ocall_shmat_t);

	ms->ms_shmid = shmid;
	if (shmaddr != NULL) {
		ms->ms_shmaddr = (const char*)__tmp;
		if (_len_shmaddr % sizeof(*shmaddr) != 0) {
			sgx_ocfree();
			return SGX_ERROR_INVALID_PARAMETER;
		}
		if (memcpy_s(__tmp, ocalloc_size, shmaddr, _len_shmaddr)) {
			sgx_ocfree();
			return SGX_ERROR_UNEXPECTED;
		}
		__tmp = (void *)((size_t)__tmp + _len_shmaddr);
		ocalloc_size -= _len_shmaddr;
	} else {
		ms->ms_shmaddr = NULL;
	}
